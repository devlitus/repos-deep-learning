...
Paso 3: Implementar sistema de recomendación basado en similitud de usuario-productos
"""
import sys
import io
import json
...
print("=" * 70)
print("👥 HYBRID COLLABORATIVE FILTERING - AMAZON FASHION")
print("=" * 70)
...
# Crear matriz user-item (usuario-producto × producto)
...
print(f"\n📐 Matriz de ratings: {ratings_matrix.shape[0]} usuario-productos × {ratings_matrix.shape[1]} productos")
...
# Calcular similitud del coseno entre usuario-productos
...
def find_similar_users(user_id, similarity_df, k=10):
    """
    Encuentra los k usuario-productos más similares a un usuario-producto dado

    Args:
        user_id: ID del usuario-producto
        similarity_df: DataFrame con similitudes
        k: Número de usuario-productos similares a retornar

    Returns:
        Series con los k usuario-productos más similares y sus similitudes
    """
    try:
...
def predict_rating(user_id, product_id, ratings_matrix, similarity_df, k=10):
    """
    Predice el rating que un usuario-producto daría a un producto
    usando el promedio ponderado de usuario-productos similares
    ...
    Args:
        ...
        k: Número de usuario-productos similares a considerar

    Returns:
        Rating predicho (float)
    """
    # Encontrar usuario-productos similares
    similar_users = find_similar_users(user_id, similarity_df, k)

    if len(similar_users) == 0:
        return 3.0

    # Obtener ratings de usuario-productos similares para este producto
    try:
...
def get_recommendations(user_id, ratings_matrix, similarity_df, n_recommendations=10, k_neighbors=10):
    """
    Obtiene las top-N recomendaciones para un usuario-producto
    ...
    Args:
        ...
        k_neighbors: Número de usuario-productos similares a considerar

    Returns:
        DataFrame con productos recomendados y ratings predichos
    """
    # Productos que el usuario-producto ya ha calificado
...
# Generar recomendaciones para algunos usuario-productos de prueba
...
for _, row in test_data.iterrows():
    ...
    # Verificar que el usuario-producto y producto existen en matrices
...
if len(actual_ratings) > 0:
    ...
    plt.tight_layout()
    plt.savefig(REPORTS_DIR / 'hybrid_predictions.png', dpi=300, bbox_inches='tight')
    print("✅ Gráfico guardado: hybrid_predictions.png")
    plt.close()

print("\n" + "=" * 70)
print("✅ HYBRID COLLABORATIVE FILTERING COMPLETADO")
print("=" * 70)
